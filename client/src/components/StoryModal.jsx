     ...
     text-white flex items-center justify-center p-4">
        <div className='w-full max-w-md'>
            ...
            relative' style={{backgroundColor: background}}>
                ...
                {mode==='text' && (
                    <textarea className='bg-transparent text-white w-full h-full p-6
                    text-lg resize-none focus:outline-none' placeholder="What's on your mind?"
                ...
                )}
                {
                    mode==='media' && previewUrl && (
                        ...
                        (
                            <video src={previewUrl} alt="" className='bject-contain max-h-full'/>
                        )
                    )
                }
            ...
            <div className='flex gap-2 mt-4'>
                <button onClick={()=> {setMode('text');setMedia(null);setPreviewUrl(null)}}className={`flex flex-1 
                items-center justify-center gap-2 p-2 rounded cursor-pointer ${mode==='text' ? "bg-white text-black" 
                : "bg-zinc-800"}`}>
                    <TextIcon size={18}/>Text
                </button>
                <label className={`flex flex-1 items-center justify-center gap-2 p-2 rounded cursor-pointer
                ${mode==='media' ? "bg-white text-black" 
                : "bg-zinc-800"}` }>
            ...
            })}className='flex flex-1 items-center justify-center gap-2 text-white py-3 mt-4 w-full rounded bg-gradient-to-r
