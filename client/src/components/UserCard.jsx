  ...
  return(
    ...
    border border-gray-200 rounded-md'>
        ...
        <div className='flex items-center justify-center gap-2 mt-4 text-xs
         text-gray-600'>
            <div className='flex items-center gap-1 border border-gray-300 rounded-full
            px-3 py-1'>
        ...
        <div className='flex mt-4 gap-2'>
            ...
            gap-2 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 
            hover:to-green-700 active:scale-95 transition text-white cursor-pointer'>
                <UserPlus  className='w-4 h-4 green-500'/> {currentUser?.following.includes(user._id) 
                ? 'Following' : 'Follow'}
            </button>
            {/* Connection Request Button / Message Button */}
            <button onClick={handleConnectionRequest} className='flex items-center
            justify-center w-16 border text-green-500 group rounded-md
            cursor-pointer active:scale-95 transition'>
             {
                currentUser?.connections?.includes(user._id) ? (
                    <MessageCircle className='w-5 h-5 green-500 group-hover:scale-105 transition'/>
                    ) : (
